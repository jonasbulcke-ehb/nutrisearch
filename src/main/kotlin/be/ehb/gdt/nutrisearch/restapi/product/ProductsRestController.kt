...
class ProductsRestController(private val service: ProductService) {
    ...
    fun putProduct(@PathVariable id: String, @RequestBody product: Product): Unit = service.updateProduct(id, product)

    @DeleteMapping("/{id}")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    fun deleteProduct(@PathVariable id: String): Unit = service.deleteProduct(id)
}